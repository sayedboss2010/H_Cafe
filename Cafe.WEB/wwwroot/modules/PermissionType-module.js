...
$(document).on('click', '.btnModalClose', function () {
    $("#addPermissionTypeModal").modal("hide");
});
...
$(document).on('click', '.btnAdd', function (e) {
    ...
    $('#PermissionTypeId').val(0);
    $("#addPermissionTypeModal").appendTo('body').modal("show");
});
...
$(document).on('click', '.edit-btn', function (e) {
    ...
    $.ajax({
        url: '/PermissionType/GetById?id=' + id,
        async: true,
...
$(document).on('click', '#btnAddEditUniv', function (e) {
    ...
    let uri = id == 0 ? '/PermissionType/Add?arName=' + arName + '&enName=' + enName + '&facs=' + facs
        : '/PermissionType/Edit?id=' + id + '&arName=' + arName + '&enName=' + enName + '&facs=' + facs;
...
$(document).on('click', '.btnDelete', function (e) {
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                $.ajax({
                    url: '/PermissionType/Delete?id=' + id,
                    async: true,
                    success: function (result) {
                        debugger;
                        if (result) {
...
$(document).on('click', '.btnPrint', function () {
    ...
    $.ajax({
        url: '/PermissionType/PrintData?term=' + term,
        async: true,
        success: function (result) {
...
//***********************************************************************/
function LoadIndex() {
    let uri = '/PermissionType/Index';

    let term = $('#SearchString').val();
    if (term != "") {
        uri = '/PermissionType/Search?term=' + term;
    }
