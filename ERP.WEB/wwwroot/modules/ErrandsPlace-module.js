...
$(document).on('click', '.btnModalClose', function () {
    $("#addErrandsPlaceModal").modal("hide");
});
...
$(document).on('click', '.btnAdd', function (e) {
    ...
    $('#txtِArAddress').val('');
    $('.divArAddress').removeClass('is-filled');

    $('#txtEnAddress').val('');
    $('.divEnAddress').removeClass('is-filled');

    $('.drpSelectFac').val(null).trigger('change');

    $('#modalTitle').html(lang == "ar" ? "إضافة" : "Add");
    $('#ErrandsPlaceId').val(0);
    $("#addErrandsPlaceModal").appendTo('body').modal("show");
});
...
$(document).on('click', '.edit-btn', function (e) {
    ...
    $.ajax({
        url: '/ErrandsPlace/GetById?id=' + id,
        async: true,
        success: function (result) {
            ...
            if (result != null && result.nameAr != "") {
                ...
                $('#txtِArAddress').val(result.AddressAr);
                $('.divArAddress').addClass('is-filled');

                $('#txtEnAddress').val(result.AddressEn);
                $('.divEnAddress').addClass('is-filled');
...
$(document).on('click', '#btnAddEditUniv', function (e) {
    ...
    let id = $('#ErrandsPlaceId').val();
    let arName = $('#txtArName').val();
    let enName = $('#txtEnName').val();
    let arAddress = $('#txtِArAddress').val();
    let enAddress = $('#txtEnAddress').val();
    ...
    let uri = id == 0 ? '/ErrandsPlace/Add?arName=' + arName + '&enName=' + enName + '&arAddress=' + arAddress + '&enAddress=' + enAddress + '&facs=' + facs
        : '/ErrandsPlace/Edit?id=' + id + '&arName=' + arName + '&enName=' + enName + '&arAddress=' + arAddress + '&enAddress=' + enAddress + '&facs=' + facs;
...
$(document).on('click', '.btnPrint', function () {
    ...
    $.ajax({
        url: '/ErrandsPlace/PrintData?term=' + term,
        async: true,
        success: function (result) {

            let arName = lang == "ar" ? "الاسم بالعربية" : "Arabic Name";
            let enName = lang == "ar" ? "الاسم بالإنجليزية" : "English Name";
            let arAddress = lang == "ar" ? "الاسم بالعربية" : "Arabic Name";
            let enAddress = lang == "ar" ? "الاسم بالإنجليزية" : "English Name";
                ...
                <table class="table table-bordered align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:center">${arAddress}</th>
                            <th style="text-align:center">${enAddress}</th>
                        </tr>
                    </thead>
                    <tbody>`;
            ...
            for (const item of result) {
                printcontent += `<tr>
                            ...
                            <td style="text-align:center">${item.AddressAr}</td>
                            <td style="text-align:center">${item.AddressEn}</td>
                        </tr>`;
            }
...
//***********************************************************************/
function LoadIndex() {
    let uri = '/ErrandsPlace/Index';

    let term = $('#SearchString').val();
    if (term != "") {
        uri = '/ErrandsPlace/Search?term=' + term;
    }
