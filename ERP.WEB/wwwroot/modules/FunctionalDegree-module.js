...
$(document).on('click', '.btnModalClose', function () {
    $("#addFunctionalDegreeModal").modal("hide");
});
...
$(document).on('click', '.btnAdd', function (e) {
    ...
    $('#FunctionalDegreeId').val(0);
    $("#addFunctionalDegreeModal").appendTo('body').modal("show");
});
...
$(document).on('click', '.edit-btn', function (e) {
    ...
    $.ajax({
        url: '/FunctionalDegree/GetById?id=' + id,
        async: true,
...
$(document).on('click', '#btnAddEditUniv', function (e) {
    ...
    let uri = id == 0 ? '/FunctionalDegree/Add?arName=' + arName + '&enName=' + enName + '&facs=' + facs
        : '/FunctionalDegree/Edit?id=' + id + '&arName=' + arName + '&enName=' + enName + '&facs=' + facs;
...
$(document).on('click', '.btnDelete', function (e) {
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                $.ajax({
                    url: '/FunctionalDegree/Delete?id=' + id,
                    async: true,
                    success: function (result) {
                        debugger;
                        if (result) {
...
$(document).on('click', '.btnPrint', function () {
    ...
    $.ajax({
        url: '/FunctionalDegree/PrintData?term=' + term,
        async: true,
        success: function (result) {
...
//***********************************************************************/
function LoadIndex() {
    let uri = '/FunctionalDegree/Index';

    let term = $('#SearchString').val();
    if (term != "") {
        uri = '/FunctionalDegree/Search?term=' + term;
    }
