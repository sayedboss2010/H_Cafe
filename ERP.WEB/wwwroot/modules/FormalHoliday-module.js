...
$(document).on('click', '.btnModalClose', function () {
    $("#addFormalHolidayModal").modal("hide");
});
...
$(document).on('click', '.btnAdd', function (e) {
    ...
    $('#FormalHolidayId').val(0);
    $("#addFormalHolidayModal").appendTo('body').modal("show");
});
...
$(document).on('click', '.edit-btn', function (e) {
    ...
    $.ajax({
        url: '/FormalHoliday/GetById?id=' + id,
        async: true,
...
$(document).on('click', '#btnAddEditUniv', function (e) {
    ...
    let uri = id == 0 ? '/FormalHoliday/Add?arName=' + arName + '&enName=' + enName + '&facs=' + facs
        : '/FormalHoliday/Edit?id=' + id + '&arName=' + arName + '&enName=' + enName + '&facs=' + facs;
...
$(document).on('click', '.btnDelete', function (e) {
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                $.ajax({
                    url: '/FormalHoliday/Delete?id=' + id,
                    async: true,
                    success: function (result) {
                        debugger;
                        if (result) {
...
$(document).on('click', '.btnPrint', function () {
    ...
    $.ajax({
        url: '/FormalHoliday/PrintData?term=' + term,
        async: true,
        success: function (result) {
...
//***********************************************************************/
function LoadIndex() {
    let uri = '/FormalHoliday/Index';

    let term = $('#SearchString').val();
    if (term != "") {
        uri = '/FormalHoliday/Search?term=' + term;
    }
